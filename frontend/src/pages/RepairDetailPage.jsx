...
export default function RepairDetailPage() {
  ...
  // Push repair context to Nova whenever repair data loads
  useEffect(() => {
    if (!repair) return;
    const title = [repair.actionDesc, repair.groupDesc, repair.componentDesc].filter(Boolean).join(' ') || `Repair ${repairId}`;
    updateContext({
      screen: 'repair_detail',
      repairId: Number(repairId),
      woNumber: repair.documentNumber || (repair.documentId != null ? String(repair.documentId) : null),
      repairTitle: title,
      repairStatus: repair.workOrderStatusCode || null,
      priority: repair.priority || null,
      shopId: technician?.shopId,
    });
  }, [repair, repairId]); // eslint-disable-line
  ...
  }, [registerActionHandler, repairId, notesSearchQuery]); // eslint-disable-line
